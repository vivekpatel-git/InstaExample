...
public class Instagram {
    ...
    public class AccessTokenTask extends AsyncTask<URL, Integer, Long> {
        ...
        protected Long doInBackground(URL... urls) {
            ...
            try {
                ...
                if (!response.equals("")) {
                    ...
                    Log.d("mytag", "jsonUser::" + jsonUser.toString());
        ...
        protected void onPostExecute(Long result) {
            progressDlg.dismiss();
            if (user != null) {
                new UserInformationTask(code, progressDlg, user).execute();
            } else {
    ...
    public class UserInformationTask extends AsyncTask<URL, Integer, Long> {
        ProgressDialog progressDlg;
        InstagramUser user;
        String code;

        public UserInformationTask(String code, ProgressDialog progressDlg, InstagramUser user) {
            this.code = code;
        ...
        protected Long doInBackground(URL... urls) {
            ...
            try {
                ...
                params.add(new BasicNameValuePair("access_token", user.accessToken));

                InstagramRequest request = new InstagramRequest();
                String response = request.get(Cons.ACCESS_USER_INFO, params);

                if (!response.equals("")) {
                    JSONObject jsonObj = (JSONObject) new JSONTokener(response).nextValue();
                    JSONObject jsonUser = jsonObj.getJSONObject("data");
                    JSONObject jsonCount = jsonUser.getJSONObject("counts");

                    user.media = jsonCount.getString("media");
                    user.follows = jsonCount.getString("follows");
                    user.followed_by = jsonCount.getString("followed_by");
